...
/*
   ...
   WARNING: This is version 1.7.1.5, with a tiny modification to allow changing the useragent in the client class.
   Also changed: added support for giving $_SERVER["HTTP_HOST"].$_SERVER["PHP_SELF"] as referrer to the target server. (Only in SSL mode)
                                Also added support for giving back the used technology...

   ===

   Version 1.7.1.5 (beta) - Sebastian Berm, 4th September 2008
   ...
   Changed in 1.7.1.5 (Sebastian Berm):
   * The classes were unable to process the removal of <?xml ... ?> at the start of the document. However xml_parser can do this as well.
...
class IXR_Message {
    ...
    function parse() {
        // first remove the XML declaration - This is not needed
    ...
    function serve($data = false) {
        if (!$data) {
            ...
            if (!$HTTP_RAW_POST_DATA) {
               die('ClientBad Request:XML-RPC API only accepts POST requests');
            }
...
class IXR_Client {
    ...
    var $error = false;
    function IXR_Client($server, $path = false, $port = 80, $timeout = false, $useragent=null) {
        if (!$path) {
        ...
        {
            $this->useragent = 'XML_RPC';
        }
        else
        {
            $this->useragent = 'IXR :: '.$useragent;
        }
 ...
 * @since 0.1.0
 */
function IXR_ClientSSL($server, $path = false, $port = 443, $timeout = false,$useragent=null) {
        parent::IXR_Client($server, $path, $port);
    ...
    {
        $this->useragent = 'IXRSSL :: '.$useragent;
    }
...
function query()
        ...
        if ($this->debug) {
            echo '<pre>'.htmlspecialchars($xml)."\n</pre>\n\n";
                ...
                $tech = (isset($_SERVER["HTTPS"])?"https://":"http://");
                curl_setopt($curl, CURLOPT_REFERER, $tech.$_SERVER["HTTP_HOST"].$_SERVER["PHP_SELF"]);
